...
- fixed per-file limits from 4 KiB to 2 MiB;
...
GPU activity is optional and appears only when an unprivileged bounded sysfs
observation exists; absence is never reported as measured zero.
...
Alpha 2 is inspection-only. There is deliberately no signal, kill, dump,
